...
最終更新: 2026-09-14 / 対象ブランチ: `claude/friendly-cerf-yzeysj`

自動テスト合計 **338 件**（Core 299 / Stt 14 / Audio 15 / App 10）。
このうち **Core 288 件と Stt 14 件は Linux 上で実行し、全件成功を確認しています**
（.NET SDK 8.0.131、2026-09-14）。Audio（`net8.0-windows`）と App（WPF）は
Windows でしか実行できないため、**GitHub Actions の `windows-latest` での結果が
このファイルに反映されるのは CI 実行後です**。

以前の記録（237 件、run #57、commit `f7232d4`）からの差分は、音質・文字起こし速度・
議事録速度の改修で追加した 101 件です（2.1 / 2.4 / 2.7）。

### CI とレビューが見つけた不具合（2026-09-14）

このブランチの CI と自動レビューで 5 件見つかり、すべて修正済みです。
**うち 3 件はこの改修が持ち込んだ実バグでした。**

| # | 内容 | 見つけた仕組み |
| --- | --- | --- |
| 1 | `FirstOrDefault(述語, 既定値)` の型推論でタプル要素名が落ち、ビルドエラー | CI（Windows ビルド） |
| 2 | コンボボックスの行型を private にしたため WPF の `DisplayMemberPath` がリフレクションで到達できない（`DisplayRows.cs` が明文化している規約違反） | CI 失敗後のコードレビュー。**ビルドは通るため、UI スモークテストでしか出ない** |
| 3 | **Catmull-Rom 補間は制御点に挟まれず最大 1.25 倍まで跳ねる。** 線形補間にはなかった性質で、ミキサーの「クリップ不可能」保証が壊れていた | Codex レビュー（P2） |
| 4 | **map-reduce の digest がコンテキスト長の検査を受けずに reduce へ渡る。** 長い会議で llama.cpp が冒頭を切り捨てる | Codex レビュー（P1） |
| 5 | **v3 設定ファイルの `Processing` が移行時に無条件で初期化される。** ピーク正規化を切っていた等の選択が消える | Codex レビュー（P2） |

3 番の修正過程で、**既存の入力クリッピング検出が偶然で動いていた**ことも判明しました
（2.1.6）。これは今回の改修以前からの不具合です。

自分のベンチマークが書いた時間順序のアサーション（`高速 <= 高精度 × 1.25`）も
CI で落ちました。**実測が示したのは、6.7 秒・1 窓・85 文字というフィクスチャでは
プロファイル間の差がウォームアップより小さいという事実**だったので、
アサーションを削除し、コードの性質（各プロファイルが宣言どおりの設定でエンジンに届くこと、
窓の数がプロファイルに依存しないこと）のみを検査するように直しました（2.4.3）。
...
#### 2.1.1 サンプルレート変換の周波数特性（今回追加）

掃引正弦波を実際に通して、変換後の振幅を入力比で測っています。
しきい値は Catmull-Rom の実測値に余裕を持たせた値で、線形補間に戻すと落ちます。

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **44.1 → 48 kHz が音声帯域を平坦に保つ** | Linux (.NET 8.0.131) | PASS | 4 kHz -0.01 / 6 kHz -0.05 / 8 kHz -0.16 dB（線形補間は -0.24 / -0.53 / -0.95 dB） |
| **16 → 48 kHz が持っている帯域を保つ** | Linux (.NET 8.0.131) | PASS | 2 kHz -0.04 / 4 kHz -0.54 / 6 kHz -2.31 dB（線形補間は -0.40 / -1.63 / -3.77 dB） |
| 変換がどの周波数も増幅しない | Linux (.NET 8.0.131) | PASS | ヘッドルームを食わないこと |
| 同一レートはバイト単位で同一 | Linux (.NET 8.0.131) | PASS | 48→48 kHz は素通し |
| 出力サンプル数が補間方式に依存しない | Linux (.NET 8.0.131) | PASS | 3次補間が先頭で数サンプル飲み込まないこと（ポンプはサンプル数で時刻を数えるため） |
| 44.1 → 48 kHz がブロック境界で連続 | Linux (.NET 8.0.131) | PASS | 441 サンプルずつと一括が 1e-5 以内で一致 |

#### 2.1.2 マルチチャンネルのモノラル化（今回追加）

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| モノラル入力は素通し | Linux (.NET 8.0.131) | PASS | 配列が厳密に一致 |
| 同相 2ch は振幅を保つ | Linux (.NET 8.0.131) | PASS | RMS 比 0.1% 以内 |
| **無相関 2ch を打ち消しと誤認しない** | Linux (.NET 8.0.131) | PASS | -3 dB は算術であって故障ではない。平均のまま |
| **逆相 2ch を検出してチャンネル 0 へ切替** | Linux (.NET 8.0.131) | PASS | 切替後の RMS が原信号の ±10% |
| 無音では何も判定しない | Linux (.NET 8.0.131) | PASS | 判定ウィンドウが閉じないこと |

#### 2.1.3 キャプチャ形式の記録と帯域制限の警告（今回追加）

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **8 / 16 kHz エンドポイントを警告する** | Linux (.NET 8.0.131) | PASS | 「復元できません」を含む文言。修復を偽装しない |
| 44.1 / 48 kHz では警告しない | Linux (.NET 8.0.131) | PASS | |
| ログブロックに必要な項目が揃う | Linux (.NET 8.0.131) | PASS | Device / SourceFormat / MixFormat / InternalFormat / Nyquist |
| 素通しか再サンプルかを明示する | Linux (.NET 8.0.131) | PASS | |

#### 2.1.4 マイクのリスニング EQ（今回追加。既定オフ）

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **傾きが宣言どおり（250 Hz と 8 kHz の差 = 設定ゲイン）** | Linux (.NET 8.0.131) | PASS | ±0.5 dB |
| **どの周波数も増幅しない** | Linux (.NET 8.0.131) | PASS | L1 ノルム正規化。7 点で確認 |
| **フルスケール入力でもフルスケールを超えない** | Linux (.NET 8.0.131) | PASS | 正弦波ではなく方形波列（インパルス応答全体を励起する入力） |
| **線形かつ時不変** | Linux (.NET 8.0.131) | PASS | 入力 20 倍で出力も厳密に 20 倍 |
| **認識用音声は EQ の有無で完全に同一** | Linux (.NET 8.0.131) | PASS | 実パイプラインで録音して比較。RMS 1% 以内 |
| **EQ は会議ファイルだけを変え、PC音声系統は変えない** | Linux (.NET 8.0.131) | PASS | 実パイプライン |
| **EQ 有効時もフルスケールに達しない** | Linux (.NET 8.0.131) | PASS | 両系統 0.9 振幅。クランプ発動サンプル数 0 |
| **認識音声 16 kHz / 会議音声 48 kHz** | Linux (.NET 8.0.131) | PASS | 実パイプライン |

#### 2.1.5 ヘッドルーム（今回追加。Codex P2 指摘の修正）

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| 基底関数の総和が常に 1（定数は定数のまま） | Linux (.NET 8.0.131) | PASS | 10,001 点 |
| **最大ゲインの実測値がミキサー定数の根拠と一致** | Linux (.NET 8.0.131) | PASS | 100,001 点を掃引し、t=0.5 で 1.25 を実測 |
| ミキサー定数がその上界の内側にある | Linux (.NET 8.0.131) | PASS | 過剰に絞っていないことも同時に検査 |
| **DC ブロッカーを含めても合計が 1.0 を超えない** | Linux (.NET 8.0.131) | PASS | ちょうど 0.4 だと 1.0013 になるため 0.39 |
| **実信号でオーバーシュートが再現し、ヘッドルームが吸収する** | Linux (.NET 8.0.131) | PASS | フルスケール方形波を 44.1→48 kHz 変換。**不具合そのものを再現してから修正を検査** |
| 実パイプラインで writer のクランプが発動しない | Linux (.NET 8.0.131) | PASS | 両系統に 1.25 振幅を同相で 1 秒。クランプ発動 0 サンプル |

#### 2.1.6 入力クリッピング検出（今回修正。既存の不具合）

以前は完成したファイルを読み直していました。動いていたのは、フルスケール 2 系統が
同相で入ったとき `0.5 + 0.5` がちょうど 1.0 になったからで、**検出ではなく偶然**でした。

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| 歪んだ入力を報告し、修復したとは言わない | Linux (.NET 8.0.131) | PASS | 加えて、**ファイル自体は歪んでいない**ことも検査（ファイルから検出したのではないことの担保） |
| **片系統だけが歪んでいても報告する** | Linux (.NET 8.0.131) | PASS | 旧方式では検出不可能だったケース |
| 正常な録音を歪みと誤検出しない | Linux (.NET 8.0.131) | PASS | 両系統 0.9 振幅 |

#### 2.1.7 帯域バランスの実測（今回追加）

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **明るい音声と鈍い音声を区別する** | Linux (.NET 8.0.131) | PASS | 高域 tilt の差 20 dB 以上を検出し、警告文を返す |
| 同等の 2 系統には何も主張しない | Linux (.NET 8.0.131) | PASS | 差 0 dB で警告なし |
| 無音は「鈍い」ではなく「測定不能」 | Linux (.NET 8.0.131) | PASS | |
...
#### 2.4.1 速度/精度プロファイル（今回追加）

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **高精度がプロファイル導入前と完全一致** | Linux (.NET 8.0.131) | PASS | `For(Accurate,…)` と `Offline(…)` を構造体比較。誰の文字起こしも劣化していないことの担保 |
| 高速が貪欲探索＋温度フォールバック無し | Linux (.NET 8.0.131) | PASS | beam 1 / inc 0 |
| 標準が両者の中間 | Linux (.NET 8.0.131) | PASS | beam 2、温度フォールバックあり |
| 未知のプロファイル値が中間に落ちる | Linux (.NET 8.0.131) | PASS | 手編集された設定ファイル対策 |
| **どのプロファイルでも日本語プロンプトを外さない** | Linux (.NET 8.0.131) | PASS | 3 プロファイル |
| 日本語以外では日本語プロンプトを付けない | Linux (.NET 8.0.131) | PASS | |
| 3 プロファイルすべてに名前と説明がある | Linux (.NET 8.0.131) | PASS | UI に出す文言 |

#### 2.4.2 窓の充填（今回追加。処理時間の最大要因）

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **窓が発話ごとではなく目標量まで充填される** | Linux (.NET 8.0.131) | PASS | **同じ音声を新旧両方の設定で実行して比較**。10 分の合成会議で 100 窓（会議長の 5.00 倍のエンコーダー換算）→ 25 窓・平均 23.7 秒（1.25 倍） |
| 窓が受容野（30 秒）を超えない | Linux (.NET 8.0.131) | PASS | エンジンに黙って切り捨てられないこと |
| **長い無音が窓に持ち込まれない** | Linux (.NET 8.0.131) | PASS | 3 秒発話 + 120 秒無音 + 3 秒発話 → 2 窓、いずれも 10 秒未満、投入合計 15 秒未満 |
| **タイムスタンプが発話時刻のまま** | Linux (.NET 8.0.131) | PASS | 窓内の音声を連続に保っていることの検査 |
| 計測がパス全体を説明する | Linux (.NET 8.0.131) | PASS | 会議長が「最大」であること（合計ではない）、ログブロックに D 章の全項目が出ること |
| **話者分離が認識と別に計測される** | Linux (.NET 8.0.131) | PASS | 「切ると速いのか」に答えられること |
| **話者分離オフでも文字起こしでき、系統は保たれる** | Linux (.NET 8.0.131) | PASS | マイク／PC音声の両方が残り、SpeakerId は null |

#### 2.4.3 実エンジンでのプロファイル比較（今回追加）

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **3 プロファイルを同一音声で実行し所要時間を表にする** | GitHub Actions / windows-latest | PASS | `TranscriptionProfileBenchmarkTests` |

CI（4論理コア・2スレッド・tiny q5_1・合成英語音声 6.7 秒）での実測:

| profile | beam | temp inc | model load | inference | total | RTF | chunks | chars |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 高速 | 1 | 0.00 | 0.11s | 2.43s | 2.56s | 0.38 | 1 | 85 |
| 標準 | 2 | 0.20 | 0.09s | 2.46s | 2.55s | 0.38 | 1 | 85 |
| 高精度 | 5 | 0.20 | 0.08s | 1.80s | 1.89s | 0.28 | 1 | 85 |

**この表からプロファイルの速度差は読み取れません。** 値は実行順（高速→標準→高精度）に
単調減少しており、ビーム幅ではなくウォームアップの順序を測っています。
6.7 秒の音声は 1 窓＝エンコーダ 1 パスで、出力も 25 トークン程度しかないため、
デコーダ側の差が初回実行の JIT・ページフォルト・クロック上昇に埋もれます。
現在はウォームアップ実行を 1 回挟んだうえで、**順序に関するアサーションは置いていません**
（フィクスチャが支えられない主張だからです）。3 系統とも出力は同一の 85 文字でした。

**このテストは精度を測りません。** 参照文字起こしが無いため、表の文字数は
「どれだけ返ってきたか」であって「どれだけ合っていたか」ではありません。
...
| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **短い会議がシングルパスになる** | Linux (.NET 8.0.131) | PASS | 推論 1 回 |
| **長い会議が map-reduce に落ちる** | Linux (.NET 8.0.131) | PASS | |
| **判定が文字数ではなくトークン数で行われる** | Linux (.NET 8.0.131) | PASS | 同じ本文を密／疎 2 つのトークナイザで判定させ、結論が変わることを確認 |
| コンテキスト長ぎりぎりまで使わない | Linux (.NET 8.0.131) | PASS | 安全マージン 12%（4096 に対し 200〜1000 トークン） |
| **ブロックが固定サイズではなく最大化される** | Linux (.NET 8.0.131) | PASS | 20000 トークンが 3〜5 ブロック（1500 文字規則なら 13 以上） |
| **行の途中で分割しない** | Linux (.NET 8.0.131) | PASS | ブロックを連結すると原文と厳密一致 |
| ブロック数の上限が効く | Linux (.NET 8.0.131) | PASS | 病的入力が無限に走らない |
| 空の文字起こしは何も計画しない | Linux (.NET 8.0.131) | PASS | |
| **digest がコンテキスト長を超える場合は畳み込む** | Linux (.NET 8.0.131) | — | Codex P1 の修正。`FoldAsync` が収まるまで要点を再要約。**実 LLM が必要なため自動テストなし**（コードレビューで確認） |
| **フォールバックのトークン数え上げが安全側に外れる** | Linux (.NET 8.0.131) | PASS | 過大評価 = 推論 1 回増、過小評価 = コンテキスト溢れ |

#### 2.7.2 構造化出力のパース（今回追加）

小型ローカルモデルが形式を外す典型パターンを網羅しています。

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| 整形された応答が正しく振り分けられる | Linux (.NET 8.0.131) | PASS | 6 セクション |
| 箇条書き記号だけを剥がす | Linux (.NET 8.0.131) | PASS | `-` / `・` / `*` |
| 見出しの装飾を吸収する | Linux (.NET 8.0.131) | PASS | `## X` / `**X**` / `X:` / `X：` / `[X]` / `【X】` の 6 通り |
| 見出しが延長されていても同じ見出し | Linux (.NET 8.0.131) | PASS | `【ToDo・ネクストアクション】` |
| 最初の見出しの前の前置きを捨てない | Linux (.NET 8.0.131) | PASS | 概要に入る |
| **見出しが 1 つも無くても議事録が出る** | Linux (.NET 8.0.131) | PASS | 全文を概要に入れ、その旨を記載 |
| **想定外の見出しでも本文を失わない** | Linux (.NET 8.0.131) | PASS | 直前のセクションに残る |
| 空／null 応答がエラーにならない | Linux (.NET 8.0.131) | PASS | |
| 見出しの重複が追記になる | Linux (.NET 8.0.131) | PASS | |
| **プロンプトとパーサが同じ見出しを見ている** | Linux (.NET 8.0.131) | PASS | プロンプト文をパーサに通して往復検証 |

#### 2.7.3 設定の移行（今回追加）

| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **v3 の設定ファイルが標準プロファイルになる** | Linux (.NET 8.0.131) | PASS | 既存の選択（モデル・形式・ビットレート）は保たれる |
| **リスニング EQ は移行時オフのまま** | Linux (.NET 8.0.131) | PASS | 根拠のない変更をしない |
| 新しい設定が保存・再読込で保持される | Linux (.NET 8.0.131) | PASS | |
...
| **文字起こしの高速化の効果量（秒単位）** | **未測定。** 窓の数が 100 → 25 に減ることは実測しました（2.4.2）が、**それが i5-1335U で何秒になるかはこの環境では測れません**（Whisper モデルを取得できないため）。実機で「同じ会議を 3 プロファイルで文字起こしし直し、`tools\Show-Performance.ps1` で比較」してください |
| **議事録の高速化の効果量（秒単位）** | **未測定。** 推論回数が N+4 → 1（または N+1）に減ることはコードとテストで確認済みですが、GGUF モデルを取得できないため所要時間は実機でのみ分かります。ログの `Minutes performance:` に出ます |
| **プロファイルごとの日本語 CER** | **未測定。** 参照文字起こしがリポジトリに無く、ビーム幅 2 を「最適」として提示していません |
| **マイクがこもる原因の確定** | **実機ログ待ち。** コード上の候補（キャプチャ形式、リサンプラー、チャンネル平均）はすべて計測・記録できるようにしましたが、**このユーザーのヘッドセットがどれに該当するかはログを見るまで分かりません** |
...
| OpenVINO / GPU 高速化 | **不採用（調査済み）。** 配布 ZIP の肥大、ggml とは別の IR モデル配布、Intel ドライバ依存、そしてエンコーダーしか速くならないこと。portable・管理者権限不要・動作PCを限定しない、という前提と衝突します。判断の詳細は `docs/ARCHITECTURE.md` §3.9 |
