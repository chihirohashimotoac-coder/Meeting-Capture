...
自動テスト合計 **327 件**（Core 288 / Stt 14 / Audio 15 / App 10）。
...
議事録速度の改修で追加した 90 件です（2.1 / 2.4 / 2.7 / 2.18）。
...
| Test | Environment | Result | Notes |
| --- | --- | --- | --- |
| **3 プロファイルを同一音声で実行し所要時間を表にする** | GitHub Actions / windows-latest | **CI 実行待ち** | `TranscriptionProfileBenchmarkTests`。CI では tiny + 合成英語音声。実機では環境変数で実音声を指定して実行 |
