...
| **whisper.cpp（Whisper.net 1.8.1 同梱）が turbo 形式を読める** | GitHub Actions / windows-latest | PASS | 実モデルをロードして推論、テキストが返る |
| **基準PC（i5-1335U）での処理時間・RAM** | **Not tested** | — | CI ランナーは 4 論理コアで、基準PCではありません |

**測定していない数値は記載しません。** カタログの `RelativeCost` は
エンコーダーの計算量（層数 × 次元²）から導いた**推定値**であり、
